...
export default function ModuleDetailDialog({ module: m, onClose }: {
  ...
  return createPortal(
    <div className="fixed inset-0 bg-black/40 z-[9999] flex items-center justify-center p-4" onClick={(e) => e.stopPropagation()}>
      <div className="w-full max-w-sm bg-card border border-border text-foreground rounded-2xl shadow-2xl flex flex-col max-h-[80vh] overflow-hidden">
        ...
        <div className="flex items-center gap-2 px-5 pt-5 pb-3 border-b border-border flex-shrink-0">
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="text-xs text-muted-foreground">{m.tasks.length} task{m.tasks.length !== 1 ? "s" : ""}</p>
          </div>
          <button onClick={onClose} className="p-1.5 rounded-lg text-muted-foreground hover:text-foreground hover:bg-muted transition-colors flex-shrink-0">
            <X size={15} />
          </button>
        ...
        <div className="overflow-y-auto flex-1 px-5 py-4 flex flex-col gap-4">
          ...
          <div>
            ...
            {m.description ? (
              ...
              <p className="text-sm text-muted-foreground/50 italic">No description added</p>
            )}
          </div>
          ...
          <div>
            ...
            {m.tasks.length === 0 ? (
              <p className="text-sm text-muted-foreground/50 italic">No tasks in this module yet</p>
            ) : (
              <div className="flex flex-col gap-2">
                {m.tasks.map((t: TaskBrief) => (
                  <div key={t.id} className="border border-border rounded-xl p-3 bg-muted/20">
                    <div className="flex items-start gap-2">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-xs text-muted-foreground mt-0.5">{STATUS_LABEL[t.status] ?? t.status}</p>
                        <div className="flex flex-wrap gap-2 mt-2">
                          {t.due_date && (
                            <span className="flex items-center gap-1 text-[11px] text-muted-foreground bg-muted px-2 py-0.5 rounded-lg">
