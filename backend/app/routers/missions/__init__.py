...
intern proposal pipeline (Phase 2B 7B-6).

Aggregate so main.py mounts one set of `/missions` routes. `admin_router`,
`design_router`, `operate_router`, and `proposals_router` MUST be included
before `student_router`: their static `/missions/admin`, `/missions/design`,
`/missions/operate`, and `/missions/proposals` would otherwise be shadowed
by student_router's dynamic `/missions/{mission_id}` — Starlette matches
routes in registration order, not by specificity, and all five are exactly
one path segment after `/missions/`.
"""
