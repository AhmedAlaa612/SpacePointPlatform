...
(Phase 2B 7B-7) + cohort-scoped instructor surface (2026-08-17).

Aggregate so main.py mounts one set of `/missions` routes. `admin_router`,
`design_router`, `operate_router`, `proposals_router`, `manager_router`,
and `instructor_router` MUST be included before `student_router`: their
static `/missions/admin`, `/missions/design`, `/missions/operate`,
`/missions/proposals`, `/missions/manager`, `/missions/library`, and
`/missions/instructor` would otherwise be shadowed by student_router's
dynamic `/missions/{mission_id}` — Starlette matches routes in
registration order, not by specificity, and all eight are exactly one path
...
from app.routers.missions.instructor import router as instructor_router
