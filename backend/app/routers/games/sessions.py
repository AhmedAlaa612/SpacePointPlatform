...
`/games/sessions/*`, gated by `require_lms_content` (same population as
`/games/admin/*` — operations + facilitator; admin passes automatically).
...
router = APIRouter(prefix="/games/sessions", tags=["games-sessions"], dependencies=[Depends(require_lms_content)])
